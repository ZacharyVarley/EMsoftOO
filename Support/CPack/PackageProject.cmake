...
set(EMsoftOO_VERSION_SHORT "${EMsoftOO_VER_MAJOR}.${EMsoftOO_VER_MINOR}")
message(STATUS "EMsoftOO_RELEASE_TYPE: ${EMsoftOO_RELEASE_TYPE}")
if("${EMsoftOO_RELEASE_TYPE}" STREQUAL "Official")
  set(EMsoftOO_VERSION_SHORT "${EMsoftOO_VERSION_MAJOR}.${EMsoftOO_VERSION_MINOR}.${EMsoftOO_VERSION_PATCH}")
elseif("${EMsoftOO_RELEASE_TYPE}" STREQUAL "Beta")
  set(EMsoftOO_VERSION_SHORT "${EMsoftOO_VERSION_MAJOR}.${EMsoftOO_VERSION_MINOR}-${EMsoftOO_RELEASE_TYPE}-${EMsoftOO_VERSION_TWEAK}")
elseif("${EMsoftOO_RELEASE_TYPE}" STREQUAL "Development")
  set(EMsoftOO_VERSION_SHORT "${EMsoftOO_VERSION_MAJOR}.${EMsoftOO_VERSION_MINOR}.${EMsoftOO_VERSION_PATCH}.${EMsoftOO_VERSION_TWEAK}")
else()
  set(EMsoftOO_VERSION_SHORT "0.0.0")
endif()

message(STATUS "EMsoftOO_VERSION: ${EMsoftOO_VERSION_MAJOR}.${EMsoftOO_VERSION_MINOR}.${EMsoftOO_VERSION_PATCH}.${EMsoftOO_VERSION_TWEAK}")

SET(CPACK_PACKAGE_DESCRIPTION_SUMMARY "EMsoftOO Tools")
SET(CPACK_PACKAGE_VENDOR "Dr. Marc De Graef")
SET(CPACK_PACKAGE_DESCRIPTION_FILE "${PROJECT_BINARY_DIR}/README.md")
SET(CPACK_RESOURCE_FILE_LICENSE "${PROJECT_BINARY_DIR}/License.txt")
SET(CPACK_PACKAGE_VERSION_MAJOR ${EMsoftOO_VER_MAJOR})
SET(CPACK_PACKAGE_VERSION_MINOR ${EMsoftOO_VER_MINOR})
SET(CPACK_PACKAGE_VERSION_PATCH ${EMsoftOO_VER_PATCH})
SET(CPACK_PACKAGE_VERSION ${EMsoftOO_VERSION})
SET(CPACK_COMPONENTS_ALL Applications Headers)
set(CPACK_COMPONENT_APPLICATIONS_DISPLAY_NAME "Applications")
set(CPACK_COMPONENT_APPLICATIONS_DESCRIPTION  "The EMsoftOO Software Tools Suite")
...
set(CPACK_PACKAGE_EXECUTABLES 1)
set(UPLOAD_FILE_NAME "")

if(APPLE)
    set(CPACK_PACKAGE_FILE_NAME "EMsoftOO-${EMsoftOO_VERSION_SHORT}-MacOS")
...
elseif(WIN32)
    if( "${CMAKE_SIZEOF_VOID_P}" EQUAL "8" )
            set(CPACK_PACKAGE_FILE_NAME "EMsoftOO-${EMsoftOO_VERSION_SHORT}-Win64")
            set(UPLOAD_FILE_NAME ${CPACK_PACKAGE_FILE_NAME}.zip)
    elseif( "${CMAKE_SIZEOF_VOID_P}" EQUAL "4" )
            set(CPACK_PACKAGE_FILE_NAME "EMsoftOO-${EMsoftOO_VERSION_SHORT}-Win32")
            set(UPLOAD_FILE_NAME ${CPACK_PACKAGE_FILE_NAME}.zip)
    else()
        set(CPACK_PACKAGE_FILE_NAME "EMsoftOO-${EMsoftOO_VERSION_SHORT}-Unknown")
        set(UPLOAD_FILE_NAME ${CPACK_PACKAGE_FILE_NAME}.zip)
    endif()
else()
    if( "${CMAKE_SIZEOF_VOID_P}" EQUAL "8" )
            set(CPACK_PACKAGE_FILE_NAME "EMsoftOO-${EMsoftOO_VERSION_SHORT}-${EMsoftOO_LINUX_SYSTEM}-x86_64")
            set(UPLOAD_FILE_NAME ${CPACK_PACKAGE_FILE_NAME}.tar.gz)
    elseif( "${CMAKE_SIZEOF_VOID_P}" EQUAL "4" )
            set(CPACK_PACKAGE_FILE_NAME "EMsoftOO-${EMsoftOO_VERSION_SHORT}-${EMsoftOO_LINUX_SYSTEM}-i386")
            set(UPLOAD_FILE_NAME ${CPACK_PACKAGE_FILE_NAME}.tar.gz)
    else()
        set(CPACK_PACKAGE_FILE_NAME "EMsoftOO-${EMsoftOO_VERSION_SHORT}-${CMAKE_SYSTEM_NAME}")
        set(UPLOAD_FILE_NAME ${CPACK_PACKAGE_FILE_NAME}.tar.gz)
    endif()
endif()


#set(EMsoftOO_WEBSITE_SERVER "EMsoftOO.bluequartz.net")
#set(EMsoftOO_WEBSITE_SERVER_PATH "/var/www/EMsoftOO.bluequartz.net")
#set(EMsoftOO_WEBSITE_SCP_USERNAME "mjackson")
...
if(WIN32)
  ...
  configure_file(${PROJECT_SOURCE_DIR}/Support/copy_nightly.bat.in
                ${EMsoftOO_BINARY_DIR}/copy_nightly.bat)
endif()
...
if(WIN32 AND NOT UNIX)
  ...
  SET(CPACK_NSIS_DISPLAY_NAME "EMsoftOO Software Tools")
  SET(CPACK_NSIS_HELP_LINK "http:\\\\\\\\EMsoftOO")
  SET(CPACK_NSIS_URL_INFO_ABOUT "http:\\\\\\\\EMsoftOO")
  SET(CPACK_NSIS_CONTACT "EMsoftOO")
  SET(CPACK_NSIS_MODIFY_PATH ON)
  SET(CPACK_GENERATOR "ZIP")
  SET(CPACK_BINARY_ZIP "ON")
  SET(CPACK_PACKAGE_INSTALL_REGISTRY_KEY "EMsoftOO Software Tools")
...
#       "${BrandedSIMPLView_DIR}/packaging/EMsoftOO_DMGBackground.tif")
#   set(CPACK_DMG_DS_STORE_SETUP_SCRIPT
#       "${PROJECT_RESOURCES_DIR}/CPack/EMsoftOO_DMGSetup.scpt")
# endif()
...
SET(CPACK_SOURCE_PACKAGE_FILE_NAME "EMsoftOO-${EMsoftOO_VERSION_SHORT}-Source")
